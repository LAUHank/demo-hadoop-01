 ...
 *
 */
public class T005_2 extends Configured implements Tool {
	/*
...
求按","号分隔的各个元素去除掉黑名单后的出现频率，输出到目录/tmp/tianliangedu/output/个人用户名的hdfs目录中。
	...
	private static class MyMapper extends Mapper<Object, Text, Text, IntWritable> {
		...
		protected void setup(
			...
			Path path = new Path(uris[0]); // 黑名单文件 一行一个单词
			String enc = "utf-8";
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(T005_2.class);
	...
	public static void main(String[] args) {
		/*
yarn jar a.jar cn.lhl.mr.xiti.T005_2 \
-Dmapreduce.job.queuename=oncourse \
lhl13-mr-xiti-T005-2 \
/user/lhl13/wordcount \
/user/lhl13/xiti_out/t005_4 \
/user/lhl13/xiti/t005/black_list_2
		 */
		int status = -1;
		try {
			status = ToolRunner.run(new T005_2(), args);
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.exit(status);
